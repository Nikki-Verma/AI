...
type AgentIntegrationDataProps = {
...
const AgentIntegrationData = ({ agentId }: AgentIntegrationDataProps) => {
  ...
  const { data, isLoading, isError, error, refetch, isRefetching } =
    useFetchData(`${config.agents.details}/${agentId}`);
  ...
  const integrateChannelHandler = async (values: UnknownObject) => {
    try {
      ...
      const payload = {
        ...values,
        pipeline_id_or_model_id: agentId,
      };
  ...
  return (
    <>
      ...
      <Tabs
        items={items(data, agentId, toggleChannelIntegrate, isRefetching)}
      />
...
export default AgentIntegrationData;
